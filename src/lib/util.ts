...
import {
  hypixelFindGuild as _hypixelFindGuild,
  hypixelGuild as _hypixelGuild,
  hypixelPlayer as _hypixelPlayer,
} from './hypixel';
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(`https://api.hypixel.net/boosters?key=${key}`)
      ...
      .then(async (json) => {
        ...
        const arr: readonly object[] = [];
        json.boosters.forEach(
          async (e: {
            readonly gameType: string;
            readonly amount: any;
            readonly length: moment.DurationInputArg1;
            readonly originalLength: moment.DurationInputArg1;
            readonly dateActivated: any;
          }) => {
            const entry = {
              game: getGametype[e.gameType],
              multiplier: `x${e.amount}`,
              remaining: _duration(e.length, 'seconds').format(
                ' m [mins], s [secs]'
              ),
              originalLength: _duration(e.originalLength, 'seconds').format(
                ' m [mins], s [secs]'
              ),
