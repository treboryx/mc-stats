...
module.exports = {
    ...
    return new Promise((resolve, reject) => {
      fetch(`https://api.hypixel.net/player?key=${key}&name=${username}`)
        ...
        .then(async json => {
          ...
          if (player.quests) Object.keys(player.quests).forEach(i => {
            if (player.quests[i].completions) completed_quests += Object.keys(player.quests[i].completions).length;
          });
          ...
          const monthly_rank = player.monthlyPackageRank ? player.monthlyPackageRank : "None";
          ...
          const unlocked_cosmetics = player.vanityMeta && player.vanityMeta.packages ? player.vanityMeta.packages.length : 0;
  ...
  hypixelFindGuild: async (uuid, key) => { // completed
    return new Promise((resolve, reject) => {
      fetch(`https://api.hypixel.net/findGuild?key=${key}&byUuid=${uuid}`)
        .then(res => res.json())
        .then(async json => {
    ...
    return new Promise((resolve, reject) => {
      fetch(`https://api.hypixel.net/guild?key=${key}&id=${id}`)
        ...
        .then(async json => {
          ...
          const preferred_games = guild.preferredGames || 0;
