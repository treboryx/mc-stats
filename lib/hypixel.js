...
module.exports = {
  ...
  hypixelFindGuild: async (uuid) => { // completed
    return new Promise((resolve, reject) => {
      fetch(`https://api.hypixel.net/findGuild?key=${keys.hypixel}&byUuid=${uuid}`)
        .then(res => res.json())
        .then(async json => {
  ...
  hypixelGuild: async (id) => {
    return new Promise((resolve, reject) => {
      fetch(`https://api.hypixel.net/guild?key=${keys.hypixel}&id=${id}`)
        .then(res => res.json())
        .then(async json => {
          ...
          if (guild.banner) {
            ...
            data.banner = { banner: guild_banner };
          }
          ...
          Object.assign(data, { coins, total_coins, created, members, name, joinable, public, experience, preferred_games, achievements, ranks, tag, description, game_experience} );
