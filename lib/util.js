      ...
      if (type === "guild") {
        fetch(`https://api.wynncraft.com/public_api.php?action=guildStats&command=${name}`)
          ...
          .then(async body => {
            ...
            async function renderBanner (layers) {
              const canvas = createCanvas(160, 320);
              const ctx = canvas.getContext("2d");
              const banner_background = await loadImage("../assets/bg_base.png");
            ...
            const banner = await renderBanner(body.banner);
            if (body.members) {
