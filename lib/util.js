  ...
  blocksmc: (query, type) => {
    if (type === "leaderboard") {
      return new Promise((resolve, reject) => {
        ...
        if (!leaderboards.includes(query)) return resolve({ errors: "Invalid game leaderboard." });
            ...
            if (game.length > 0 ) {
              ...
              $(this).find("tbody").each(function () {
                if (game === "recent_battles") {
                  ...
                  $(this).find("tr").each(function () {
                    $(this).find("td").each(function () {
                      ...
                      if (text.match(/[0-9]{2}:[0-9]{2}:[0-9]{2}/)) recent_battles.push("newLine");
                ...
                $(this).find(".stats-table-field").each(function () {
                  ...
                  if (objName === "games_played") data.totalGamesPlayed += objValue;
    ...
    return new Promise((resolve, reject) => {
      ...
      if (type === "player") {
        fetch(`https://api.wynncraft.com/v2/player/${name}/stats`)
          ...
          .then(async body => {
            body = JSON.parse(body);
            if (body.code === 404 || (body.code === 400 && body.message === "Bad Request") || !body.data || body.data.length === 0) {
              return resolve({
  ...
  mccentral: (query, type) => {
    if (type === "leaderboard") {
      return new Promise((resolve, reject) => {
        ...
        if (!leaderboardName) return resolve({ errors: "Invaid game leaderboard." });
