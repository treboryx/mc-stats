...
export function hypixelPlayer(username, key) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
            fetch(`https://api.hypixel.net/player?key=${key}&name=${username}`)
                ...
                if (player.prefix)
                    rankName = player.prefix.replace(/\[|\]/g, "").replace(/\xA7[0-9A-FK-OR]/ig, "");
                if (player.rankPlusColor)
                    rankPlus = player.rankPlusColor;
                ...
                const total_votes = player.voting && player.voting.total ? player.voting.total : 0;
                const vanity_tokens = player.vanityTokens || 0;
                const unlocked_cosmetics = player.vanityMeta && player.vanityMeta.packages
                ...
                if (player.parkourCompletions) {
                    Object.keys(player.parkourCompletions).forEach((i) => {
                        parkour_completions[i] = {
                            time: parkourTime(Math.min(...player.parkourCompletions[i].map((value) => value.timeTook))),
                ...
                Object.assign(data.player, { completed_quests, achievement_points, achievements, achievements_one_time, rankName, rankPlus, first_login, last_login, last_logout, networkExp, pending_friend_requests, karma, parkour_completions, vanity_tokens, unlocked_cosmetics, chat_channel, recent_minecraft_version, total_rewards, total_daily_rewards, total_votes, total_tokens, reward_streak, reward_score, reward_high_score, language, rank_plus_colour, most_recent_game });
                switch (true) {
                    ...
                    case player.stats.HungerGames:
                        ...
                        switch (true) {
                            ...
                            case blitz["games_played_shadow knight"]:
                                const blitz_shadow_knight_kills = blitz["kills_shadow knight"] || 0;
                                const blitz_shadow_knight_wins = blitz["wins_shadow knight"] || 0;
                                const blitz_shadow_knight_wins_teams = blitz["wins_teams_shadow knight"] || 0;
                                const blitz_shadow_knight_damage = blitz["damage_shadow knight"] || 0;
                                const blitz_shadow_knight_damage_taken = blitz["damage_taken_shadow knight"] || 0;
                                const blitz_shadow_knight_potions_drunk = blitz["potions_drunk_shadow knight"] || 0;
                                const blitz_shadow_knight_potions_thrown = blitz["potions_thrown_shadow knight"] || 0;
                                const blitz_shadow_knight_mobs_spawned = blitz["mobs_spawned_shadow knight"] || 0;
                                const blitz_shadow_knight_chests_opened = blitz["chests_opened_shadow knight"] || 0;
                                const blitz_shadow_knight_arrows_hit = blitz["arrows_hit_shadow knight"] || 0;
                                const blitz_shadow_knight_arrows_fired = blitz["arrows_fired_shadow knight"] || 0;
                                const blitz_shadow_knight_games_played = blitz["games_played_shadow knight"] || 0;
                                const blitz_shadow_knight_time_played = blitz["time_played_shadow knight"] || 0;
                            ...
                            case blitz["games_played_hype train"]:
                                ...
                                const blitz_hype_train_wins_teams = blitz["wins_teams_hype train"] || 0;
                                const blitz_hype_train_damage = blitz["damage_hype train"] || 0;
                                const blitz_hype_train_damage_taken = blitz["damage_taken_hype train"] || 0;
                                const blitz_hype_train_potions_drunk = blitz["potions_drunk_hype train"] || 0;
                                const blitz_hype_train_potions_thrown = blitz["potions_thrown_hype train"] || 0;
                                const blitz_hype_train_mobs_spawned = blitz["mobs_spawned_hype train"] || 0;
                                const blitz_hype_train_chests_opened = blitz["chests_opened_hype train"] || 0;
                                const blitz_hype_train_arrows_hit = blitz["arrows_hit_hype train"] || 0;
                                const blitz_hype_train_arrows_fired = blitz["arrows_fired_hype train"] || 0;
                                const blitz_hype_train_games_played = blitz["games_played_hype train"] || 0;
                                const blitz_hype_train_time_played = blitz["time_played_hype train"] || 0;
                    ...
                    case player.stats.Pit:
                        ...
                        const pit_renown_unlocks = pit.profile.renown_unlocks ? pit.profile.renown_unlocks.length : 0;
                        ...
                        const pit_recent_kill = pit.profile.recent_kills ? pit.profile.recent_kills[0] : 0;
...
export function hypixelFindGuild(search, type, key) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
...
export function hypixelGuild(id, key) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
